...
if config.experiment == 1:
    ...
    n_stages = 8
    exp_name = "castle_SKROCK_s" + str(n_stages)
    # setup the operators
    ...
    n_stages = 12
    exp_name = "person_SKROCK_s" + str(n_stages)
    # setup the operators
    A, AT, AAT_norm, min_ev, max_ev = motion_blur_operators(
    ...
    n_stages = 8
    exp_name = "lizard_SKROCK_s" + str(n_stages)
    # setup the operators
    A, AT, AAT_norm, min_ev, max_ev = motion_blur_operators(
...
skrock_sample = img_torch_blurry
post_var = st.welford(skrock_sample)
post_fft_var = st.welford(torch.real(torch.fft.fft2(skrock_sample)))
post_abs_fft_var = st.welford(torch.fft.fft2(skrock_sample).abs())

mean = post_var.get_mean()
var = post_var.get_mean()

exp_name += "_" + str(n_iter) + "n_iter"
...
for i in tqdm(range(n_iter)):
    ...
    skrock_sample = skrock_one_step(skrock_sample, n_stages, 
                                    Lip_total, eta, delta_perc, grad_f)

    #collect stats every iter
    logpi_vals[i] = f(skrock_sample).item()
    post_var.update(skrock_sample)
    post_fft_var.update(torch.real(torch.fft.fft2(skrock_sample)))
    post_abs_fft_var.update(torch.fft.fft2(skrock_sample).abs())

    #collect stats every "save_iter"
    if np.mod(i, save_iter) == 0:
        samples[cnt] = skrock_sample.detach().clone().cpu()
        mean = post_var.get_mean()
...
fig, ax = plt.subplots()
ax.set_title(f"Deblurred Image SKROCK MMSE (Regularization Cost {model.cost(mu*mean)[0].item():.1f}, PSNR: {peak_signal_noise_ratio(mean, img_torch, data_range=1.0).item():.2f})")
...
fig, ax = plt.subplots()
ax.set_title(f"SKROCK variance")
