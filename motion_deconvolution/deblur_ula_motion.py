...
ula_sample = img_torch_blurry
post_var = st.welford(ula_sample)
post_fft_var = st.welford(torch.real(torch.fft.fft2(ula_sample)))
post_abs_fft_var = st.welford(torch.fft.fft2(ula_sample).abs())
...
for i in tqdm(range(n_iter)):
    ...
    ula_sample = ula_one_step(ula_sample, Z_k, h_ula, grad_f)

    #collect stats every iter
    logpi_vals[i] = f(ula_sample).item()
    post_var.update(ula_sample)
    post_fft_var.update(torch.real(torch.fft.fft2(ula_sample)))
    post_abs_fft_var.update(torch.fft.fft2(ula_sample).abs())

    #collect stats every "save_iter"
    if np.mod(i, save_iter) == 0:
        samples[cnt] = ula_sample.detach().clone().cpu()
        mean = post_var.get_mean()
...
fig, ax = plt.subplots()
ax.set_title(f"Deblurred Image ULA MMSE (Regularization Cost {model.cost(mu*mean)[0].item():.1f}, PSNR: {peak_signal_noise_ratio(mean, img_torch, data_range=1.0).item():.2f})")
